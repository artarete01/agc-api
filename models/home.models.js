...
home.show_home_group_product = (data, result) => {
  let sql = ` select * from home_group_product`;
  // sql += `where dl = 0 order by  create_date DESC`;
  connection.query(sql, (err, res) => {
    if (err) {
      response = {
...
home.update_home_group_product = (data, result) => {
  console.log(data);
  let obj = {
  ...
  console.log(obj);
  connection.query(`update home_group_product set ? where id = ${data.id}`, [obj], (err, res) => {
    if (err) {
      response = {
        code: 99,
        message: err,
